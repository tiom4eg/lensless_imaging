...
def log_git_commit_and_patch(save_dir):
    ...
    """
    in_repo = subprocess.call(
        ["git", "rev-parse", "--is-inside-work-tree"],
        stdout=subprocess.DEVNULL,
        stderr=subprocess.DEVNULL,
    ) == 0
    if not in_repo:
        print("Not a git repository, skipping git commit/patch logging.")
        return
    ...
    with commit_path.open("w") as f:
        subprocess.call(["git", "rev-parse", "HEAD"], stdout=f, stderr=subprocess.DEVNULL)
    with patch_path.open("w") as f:
        subprocess.call(["git", "diff", "HEAD"], stdout=f, stderr=subprocess.DEVNULL)
