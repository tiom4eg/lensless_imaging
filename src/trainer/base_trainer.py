...
class BaseTrainer:
    ...
    def __init__(
        ...
        self.is_main = accelerator.is_main_process if accelerator is not None else True
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, batch in enumerate(
            tqdm(self.train_dataloader, desc="train", total=self.epoch_len, disable=not self.is_main)
        ):
            try:
                ...
                is_distributed = self.accelerator is not None and self.accelerator.num_processes > 1
                if self.skip_oom and not is_distributed:
    ...
    def _save_checkpoint(self, epoch, save_best=False, only_best=False):
        ...
        """
        # In distributed runs only the main process writes and the model is unwrapped first, so the state dict is clean
        ...
        if not (only_best and save_best):
            ...
            if self.config.writer.log_checkpoints and self.writer is not None:
                self.writer.add_checkpoint(filename, str(self.checkpoint_dir.parent))
            self.logger.info(f"Saving checkpoint: {filename} ...")
            if self.hf_uploader is not None:
                self.hf_uploader.upload(filename, commit_message=f"checkpoint epoch {epoch}")
        if save_best:
            ...
            if self.hf_uploader is not None:
                self.hf_uploader.upload(best_path, commit_message=f"best @ epoch {epoch}")
    ...
    def _unwrapped_model(self):
        """Return the underlying model, unwrapping the accelerate/DDP wrapper."""
        if self.accelerator is not None:
